...
The property that all siblings have the same `due` date is true when
...
* if a card is reordered while its sibling is not (because they are in
  different decks)
...
at least two ways. By adding/removing text in a field, or by editing the
