...
where cards.id in %s)"""
            ...
            # fields
            row.extend(
                [self.processText(fieldContent) for fieldContent in splitFields(flds)]
            )
            # tags
            if self.includeTags:
...
class AnkiPackageExporter(AnkiExporter):
    ...
    def doExport(self, zip: ZipFile, path: str) -> Dict[str, str]:  # type: ignore
    ...
    def _exportMedia(self, zip: ZipFile, files: List[str], fdir: str) -> Dict[str, str]:
        media = {}
