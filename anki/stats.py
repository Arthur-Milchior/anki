...
select count(), avg(ivl), max(ivl) from cards where did in %s and queue = {QUEUE_REV}""" %
    ...
    def easeGraph(self):
        ...
        txt += self._graph(id="ease", data=[
            dict(data=d['lrn'], color=self.col.conf.get("colors", defaultColors)["learn"], label=_("Learning")),
            dict(data=d['yng'], color=self.col.conf.get("colors", defaultColors)["young"], label=_("Young")),
            dict(data=d['mtr'], color=self.col.conf.get("colors", defaultColors)["mature"], label=_("Mature")),
            ], type="bars", conf=dict(
