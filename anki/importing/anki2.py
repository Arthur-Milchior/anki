...
class Anki2Importer(Importer):
    ...
    def _mid(self, srcMid):
        ...
        mid = srcMid
        srcModel = self.src.models.get(srcMid, orNone=False)
        ...
        while True:
            ...
            if not self.dst.models.have(mid):
                ...
                dstModel['ls'] = self.col.ls
                dstModel.update()
                break
            # there's an existing model; do the schemas match?
            dstModel = self.dst.models.get(mid, orNone=False)
    ...
    def _did(self, did):
        ...
        if self.deckPrefix:
            if not importedDeck.isTopLevel():
    ...
    def _importCards(self):
        ...
        for card in self.src.db.execute(
            ...
            if queue in (CARD_DUE, CARD_FILTERED) or type == QUEUE_REV:
                due -= aheadBy
