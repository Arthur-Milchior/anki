...
<button title="%s" id=ansbut onclick='pycmd("ans");'>%s</button>''' % (
    ...
    def _remaining(self):
        ...
        listColors = ["new", "learn", "rev"]
        return " + ".join(f'<font color="{self.mw.col.conf.get("colors", defaultColors)[color]}">{counts[idx]}</font>' for idx, color in enumerate(listColors))
    ...
    def _answerButtonList(self):
        ...
        if cnt == 2:
            return buttons + ((2, _("Good")),)
        elif cnt == 3:
            return buttons + ((2, _("Good")), (3, _("Easy")))
        else:
            return buttons + ((2, _("Hard")), (3, _("Good")), (4, _("Easy")))
...
%s</button></td>''' % (due, extra, _("Shortcut key: %s") % ease, ease, ease, label)
